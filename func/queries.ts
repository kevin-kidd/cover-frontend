import type { Collection, Snip721Token, Token } from "../stores/AssetStore";
...
export const getSnip20Balance = async (
	...
	try {
		const tokenInfo = await client.query.snip20.getSnip20Params({
...
export const getSnip721Inventory = async (
	client: SecretNetworkClient,
	collection: Collection,
) => {
	try {
		const inventory: Snip721Token[] = [];
		...
		console.error(
			`Unable to find query token IDs for contract: ${collection.details.contract.address}`,
		);
	} catch (e) {
		console.error(e);
	}
