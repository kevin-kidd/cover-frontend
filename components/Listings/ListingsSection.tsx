		...
		<div className="search-box w-full h-8 bg-[#0E1E2C] group">
			<div className="inline-flex w-full h-full items-center ml-3 sm:ml-4">
				...
				<svg
					...
					onKeyDown={(e) => e.key === "Enter" && setTooltipOpen(!tooltipOpen)}
...
const AdvancedButton: FunctionComponent = () => {
	return (
		<button
			type="button"
			className="w-fit bg-[#FF6767] py-5 px-6 gap-2 flex items-center rounded-xl text-white text-sm font-light hover:bg-[#ff6f6f]"
		>
			<svg
				viewBox="0 0 21 22"
				className="w-5 h-5"
...
const SectionTitle: FunctionComponent<{ title?: string }> = ({ title }) => {
	const tokenToggle = usePersistentStore(
		(state) => state.config.toggles.tokenToggle,
	);
	const listingToggle = usePersistentStore(
		(state) => state.config.toggles.listingToggle,
	);

	if (title === undefined) {
		title = `${
			tokenToggle === "Tokens" ? "Token" : "NFT"
		} ${listingToggle}ing Listings`;
	}
	...
	return (
		<div className="flex justify-between items-end pt-4 px-1 sm:px-2 md:px-3 xl:px-6">
			...
			<a
				href="/#"
				className="mt-1 font-medium text-red-500 transition duration-150 text-sm lg:text-tiny 2xl:text-base"
			>
				All Listings
			</a>
		</div>
...
const SectionHeader: FunctionComponent<{ displayToggles: boolean }> = ({
	displayToggles,
}) => {
	...
	return (
		<>
			{/* Mobile */}
			<div className="lg:hidden flex justify-evenly items-center mt-4 gap-x-3">
				<div className="flex flex-col max-w-xs items-center w-full gap-2">
...
export const ListingsSection: FunctionComponent<{
	listings: Listing[];
	title?: string;
	displayFilters: boolean;
}> = ({ listings, title, displayFilters }) => {
	return (
