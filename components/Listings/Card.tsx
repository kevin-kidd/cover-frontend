...
import type { Listing } from "../../types/general";
		...
		return (
			<div className="border-b border-[#8B98FF] flex flex-col w-full py-1">
				<div className="pt-1 px-2 w-fit mx-auto border-b border-[#535C95] inline-flex justify-center items-end py-1">
					<p className={`text-white ${classes.text.medium}`}>
						{listing.borrowing.amount}
					</p>
					<p className={`text-white pl-1 ${classes.text.small}`}>
						{listing.borrowing.name}
					</p>
					<p className={`text-gray-400 pb-1 pl-1 ${classes.text.smaller}`}>
						≈${listing.borrowing.estimatedValue}
					</p>
				</div>
				<p
					className={`text-[#86DD85] flex font-light justify-center pt-1 ${classes.text.small}`}
				>
					Interest Rate of {listing.returnPercentage}%
				</p>
				<p
					className={`text-white font-light pl-1 pb-1 flex justify-center ${classes.text.smaller}`}
				>
			...
			{listing.lending.total ? (
				<>
					<div className="w-full inline-flex justify-center items-end">
						<p className={`text-white pl-1 sm:pl-2 ${classes.text.medium}`}>
							{listing.lending.amount} / {listing.lending.total}
						</p>
						<p className={`text-white pl-1 sm:pl-2 ${classes.text.smaller}`}>
							{listing.lending.name}
						</p>
					</div>
					{/* Progress Bar */}
					<div className="flex justify-center">
						<div className="w-11/12 lg:w-10/12 rounded-full h-2 desktop:h-2.5 big:h-3 4k:h-5 my-1 4k:my-3 progress-bar-track">
							<div
								className="progress-bar h-2 desktop:h-2.5 big:h-3 4k:h-5 rounded-full"
								style={{
									width: `${
										(1 - listing.lending.amount / listing.lending.total) * 100
									}%`,
								}}
							/>
						</div>
				...
				<div className="w-full inline-flex justify-center items-center">
					<p className={`text-white ${classes.text.medium}`}>
						{listing.lending.amount}
					</p>
					<p className={`text-white pl-1 ${classes.text.smaller}`}>
						{listing.lending.name}
					</p>
					<p className={`text-gray-400 pb-1 pl-1 ${classes.text.smaller}`}>
						≈${listing.lending.estimatedValue}
					</p>
				</div>
			)}
			<p
				className={`text-white font-light justify-center flex ${classes.text.smaller}`}
			>
				for {listing.duration}
			</p>
		</div>
		...
		return (
			<div className="mt-1 flex flex-col">
				<div className="w-full flex justify-center items-center">
					<p className={`collateralized-by ${classes.text.medium}`}>
						Collateralized by
					</p>
				</div>
				<div className="w-full inline-flex justify-center items-center pt-1 4k:pt-3">
					<p className={`text-white truncate ${classes.text.small}`}>
						{listing.collateral.amount}x {listing.collateral.name}
					</p>
					<div className="w-6 h-6 rounded-full bg-black ml-2 relative">
						<Img
							src={`https://res.cloudinary.com/drgbtjcgt/image/fetch/${listing.collateral.icon}`}
							placeholder="empty"
							className="rounded-full border-white border"
							alt={`icon-${listing.collateral.name}`}
							priority={true}
							width="100"
							height="100"
						/>
	...
	return (
		<div className="h-full w-full sm:px-2 flex flex-col justify-center">
			<div className="w-full pt-2 pb-1 inline-flex justify-center items-end">
				...
				<p className={`pl-1 sm:pl-2 text-[#FF6969] ${classes.text.small}`}>
					+{listing.returnPercentage}% return
				</p>
			</div>
				...
				{listing.collateral.tokenType === "snip721" ? (
					<>
						<p className={`md:pl-2 pl-1 text-white ${classes.text.small}`}>
							{listing.collateral.amount} {listing.collateral.name}
						</p>
						<p
							className={`md:pl-2 pl-1 card-collateral ${classes.text.smaller}`}
						>
							as collateral
						</p>
					</>
					...
					<>
						{listing.collateral.amount < 1 ? (
							<>
								<p className={`md:pl-2 pl-1 text-white ${classes.text.small}`}>
									1 {listing.collateral.name}
								</p>
								<p
									className={`md:pl-2 pl-1 card-collateral ${classes.text.smaller}`}
								>
									per {collateralPerAmount} {listing.lending.name}
								</p>
							</>
						) : (
							<>
								<p className={`md:pl-2 pl-1 text-white ${classes.text.small}`}>
									{roundedCollateral === listing.collateral.amount
										? `${listing.collateral.amount}`
										: `~${roundedCollateral}`}{" "}
									{listing.collateral.name}
								</p>
								<p
									className={`md:pl-2 pl-1 card-collateral ${classes.text.smaller}`}
								>
...
const MultiImage: FunctionComponent<Snip721DisplayProps> = ({
	images,
	setLoading,
}) => {
	if (images.length > 2) {
		return (
			...
			>
				...
				<SwiperSlide className="text-white rounded bg-black">Test</SwiperSlide>
			</Swiper>
		);
	}
	return (
...
export const CardFooter: FunctionComponent<{
	listing: Listing;
	setLoading: Dispatch<SetStateAction<boolean>>;
}> = ({ listing, setLoading }) => {
	let imageDisplay: ReactElement;
	if (listing.listingType === "borrow") {
		if (listing.collateral.images.length > 0) {
			if (listing.collateral.images[0].length > 0) {
				if (listing.collateral.images.length > 1)
					imageDisplay = (
						<MultiImage
							setLoading={setLoading}
							images={listing.collateral.images}
						/>
					);
				else
					imageDisplay = (
						<Img
							...
							src={
								listing.collateral.images[0].includes("data:image/")
									? listing.collateral.images[0]
									: `https://res.cloudinary.com/drgbtjcgt/image/fetch/${listing.collateral.images[0]}`
							}
						/>
					);
			}
		...
		return (
			<div className="w-auto h-auto p-3 mb-2 aspect-square">{imageDisplay}</div>
		);
	}
	...
	return (
		<div className="h-auto w-auto p-2 lg:p-3 flex items-end">
			<div className="w-3/4 aspect-square mx-auto relative">
				<div className="rounded-full z-10 bg-white absolute border border-white border-double w-2/5 h-auto">
					<Image
						src={`https://res.cloudinary.com/drgbtjcgt/image/fetch/${listing.collateral.images[0]}`}
						width="100"
						height="100"
						alt={listing.collateral.name}
						priority={true}
						placeholder="empty"
						className="rounded-full border-white border-2 w-full h-full"
						layout="responsive"
					/>
				</div>
				<div className="rounded-full block w-full border-double border-white border-4 2xl:border-6">
					<Image
						layout="responsive"
						width="500"
						height="500"
...
export const Card: FunctionComponent<{ listing: Listing; index: number }> = ({
	listing,
	index,
}) => {
	...
	return (
		<Link href={loading ? "#" : `/listing/${listing.address}`} legacyBehavior>
			<div
				className={`w-44 sm:w-52 h-full mx-auto ${hiddenClass} ${
