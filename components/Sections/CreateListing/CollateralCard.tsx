...
export const CollateralCard: FunctionComponent = () => {
		...
		const getBalance = async (token: Token) => {
			...
			if (tokenBalance) {
				setCollateralBalance(
					tokenBalance.type === "native"
						? tokenBalance.unwrapped.balance
	...
	}, [collateralState.token, client, createListingToggle, tokenBalances.find]);
			...
			<div className="bg-[#1A2128] w-full max-w-md mx-auto h-full rounded-lg px-10 py-6 flex flex-col items-center relative">
				...
				{!collateralState.type && (
					<div className="mt-4 mb-6 w-full flex flex-col items-center h-full justify-center">
						...
						<p className="py-3 text-white text-lg">or</p>
						<CollectionSearchBox />
					</div>
				)}
				...
				{collateralState.type === "snip20" && (
					<div className="flex flex-col gap-y-2 w-full items-center relative bg-[#13181d] rounded-lg p-2 mb-4 mt-4">
						<ArrowLeftIcon
							className="w-5 h-5 absolute left-2 top-2 hover:cursor-pointer"
						...
						<div className="font-semibold">
							...
							<div className="font-normal inline-flex ml-1 items-end">
								{collateralBalance === undefined ? (
									<svg
										className="animate-spin h-4 w-4 text-white ml-1"
										...
										{collateralState.token.permit ? (
											...
											>
												<svg
													...
													viewBox="0 0 24 24"
													fill="currentColor"
													className="h-5 -mb-1 fill-red-400 hover:cursor-pointer"
												>
											...
											<div
												className="tooltip tooltip-error flex justify-center ml-1"
												data-tip="Create a permit"
											>
												<Image
													src="/static/lock.svg"
													alt="Unlock"
				...
				<div className="grid grid-flow-col auto-cols-max gap-x-4 text-white mt-3">
					{collateralState.type === "snip20" ||
					(collateralState.type && createListingToggle === "Lending") ? (
						<>
							<div className="w-full flex justify-center gap-x-4 text-white">
								...
								<input
									type="number"
									value={
										collateralState.amount === -1 ? "" : collateralState.amount
									}
									...
									onChange={(e) => {
										...
										collateralBalance &&
										Number(e.target.value) > collateralBalance
											? collateralState.setAmount(collateralBalance)
											: collateralState.setAmount(Number(e.target.value));
									}}
