...
export const TermsCard: FunctionComponent = () => {
	...
	const borrowingTerms: ReactElement = (
		<>
			...
			<span className="font-semibold">
				...
				{createListingState.principal.type === "snip20"
					? ` ${createListingState.principal.token.details.name} `
					: "Collection Name"}
			</span>
			(principal) + up to
			<span className="font-semibold">
				{` ${round(
					createListingState.principal.amount *
						(createListingState.returnAmount.percentage / 100),
					2,
				).toFixed(2)}`}{" "}
				{createListingState.principal.type === "snip20"
					? createListingState.principal.token.details.name
			...
			(interest) in at most{" "}
			<span className="font-semibold">
				{createListingState.duration.days} days
			</span>
			. The interest owed is applied in hourly increments. If you fail to repay
			the loan, you will lose your collateral.
		</>
	);
	...
	const lendingTerms: ReactElement = (
		<>
			...
			. If they fail to return the principal + interest owed, you will receive
			the collateral.
		</>
	);
			...
			<div className="bg-[#1A2128] w-full max-w-md mx-auto rounded-lg px-8 py-6 flex flex-col items-center relative">
				...
				<div className="form-control">
					<label className="cursor-pointer label">
						<input
							type="checkbox"
							checked={misc.signedTerms}
							onChange={() => misc.setSignedTerms(!misc.signedTerms)}
							className="checkbox checkbox-accent bg-transparent focus:ring-0 focus:ring-offset-0 focus:outline-none"
						/>
						<span className="label-text ml-2 text-white text-sm">
			...
			<div className="bg-[#1A2128] w-full h-full max-w-md mx-auto rounded-lg px-10 py-6 flex flex-col items-center relative">
				...
				<div className="flex flex-col my-auto">
					...
					<div className="flex justify-between mx-auto w-1/2 mt-6">
						...
						<button
							type="button"
							className="py-2 px-4 bg-transparent border-[#5AAD52] border rounded-lg hover:text-white duration-200 transition"
						>
							Yes
						</button>
					</div>
