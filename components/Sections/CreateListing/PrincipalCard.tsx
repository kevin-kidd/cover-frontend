...
export const PrincipalCard: FunctionComponent = () => {
	...
	}, [principalState.token, client, tokenBalances, createListingToggle]);
					...
					{principalState.type === "snip20" ? (
						<>
							...
							<input
								type="number"
								value={
									principalState.amount === -1 ? "" : principalState.amount
								}
								disabled={!client}
								...
								onChange={(e) => {
									createListingToggle === "Lending" &&
									principalBalance &&
									Number(e.target.value) > principalBalance
										? principalState.setAmount(principalBalance)
										: principalState.setAmount(Number(e.target.value));
								}}
					...
					{principalState.type && (
						<>
							...
							<input
								...
								min="3"
								max="999"
								value={duration.days === -1 ? "" : duration.days}
								disabled={!client}
								className={classNames(
