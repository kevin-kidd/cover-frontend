        ...
        <div className="w-full flex flex-row gap-x-3 mb-1 justify-center">
            {
                links.map((link) => (
                    <a key={link.href} href={link.href} target="_blank" rel="noreferrer">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox={link.viewBox}
    ...
    const balance: ReactElement = (
        <div className="flex flex-row items-center justify-center w-full gap-x-2 px-6">
            ...
            <div className="flex flex-col w-3/4 lg:w-full">
                <div className="tooltip hover:cursor-pointer has-tooltip flex justify-center"
                     data-tip={isCopied ? "Copied!" : "Copy to clipboard"}
                     onMouseLeave={() => setIsCopied(false)}
                     onClick={() => {
                         document.addEventListener('copy', function(e) {
                             e.clipboardData.setData('text/plain', wallet.address);
                             e.preventDefault();
                         }, true);
