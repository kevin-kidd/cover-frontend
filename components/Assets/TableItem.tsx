...
import { type Token, useAssetStore } from "../../stores/AssetStore";
...
export const TableItem: FunctionComponent<{ token: Token }> = ({ token }) => {
	...
	const tokenBalance = useMemo(
		() =>
			tokenBalances.find(
				(tokenBalance) =>
					tokenBalance.address === token.details.contract.address,
			),
		[tokenBalances, token.details.contract.address],
	);
	...
	const handlePermit = async () => {
		...
		if (
			snip20.tokens?.some(
				(token) =>
					token.details.contract.address === newToken.details.contract.address,
			)
		) {
		...
		<>
			<div className="justify-center items-center flex col-start-2 h-8 w-8 sm:h-11 sm:w-11">
				<Image
					src={
						token.details.icon ??
						"https://res.cloudinary.com/drgbtjcgt/image/upload/v1663144475/unknownToken_zfyslv.svg"
					}
			...
			<div className="col-start-3 col-span-3 xl:col-span-2 flex flex-col items-center">
				<p className="text-white text-sm sm:text-base">{token.details.name}</p>
	...
	const depositWithdrawOptions = {
		sSCRT: (
			<>
				<p
					className="text-[#7BBD75] text-sm hidden md:block cursor-pointer"
					onClick={() => openPopup("wrap")}
					onKeyDown={(e) => e.key === "Enter" && openPopup("wrap")}
				>
				...
				<p
					className="text-[#7BBD75] text-xs md:hidden cursor-pointer"
					onClick={() => openPopup("wrap")}
					onKeyDown={(e) => e.key === "Enter" && openPopup("wrap")}
				>
					Wrap / Unwrap
		...
		ibc: (
			<>
				<p
					...
					onKeyDown={(e) => e.key === "Enter" && openPopup("deposit")}
				>
					Deposit via IBC
				</p>
				<p
					className="text-[#7BBD75] text-sm hidden md:block cursor-pointer"
					onClick={() => openPopup("withdraw")}
					onKeyDown={(e) => e.key === "Enter" && openPopup("withdraw")}
				>
					Withdraw via IBC
				</p>
				<p
	...
	const balanceNotFound: ReactElement = (
		<div
			...
			data-tip="Failed to fetch balance."
			onClick={handlePermit}
			onKeyDown={(e) => e.key === "Enter" && handlePermit()}
		>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				viewBox="0 0 24 24"
				fill="currentColor"
				className="h-10 mt-1.5 fill-red-400 hover:cursor-pointer"
			>
				<title>Failed to fetch balance.</title>
				<path
					fillRule="evenodd"
		...
		<div className="w-full bg-[#1A2128] rounded-xl mt-3 relative sm:py-2">
			<div className="grid grid-cols-3 md:grid-cols-4 place-items-center h-12">
				...
				{token.isImported && (
					<button
						type="button"
						className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 bg-black p-0.5 rounded-md absolute -right-1 -top-1"
					>
