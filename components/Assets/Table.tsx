...
import { assets, type Token, useAssetStore } from "../../stores/AssetStore";
...
const tableHeader: ReactElement = (
	<div className="w-full bg-[#171d24] rounded-xl py-2">
		<div className="grid grid-cols-3 md:grid-cols-4 place-items-center">
			...
			<p className="text-[#B2BFCD] text-sm font-semibold md:hidden">Transfer</p>
		</div>
	</div>
);
...
export const Table: FunctionComponent = () => {
	...
	const hideZeroToggle = usePersistentStore(
		(state) => state.config.toggles.hideZeroBalances,
	);
	...
	useEffect(() => {
		const getBalance = async (token: Token) => {
			...
			if (
				tokenBalances.some(
					(tokenBalance) => tokenBalance.address === newTokenBalance.address,
				)
			) {
		...
		<div className="flex flex-col pt-3 sm:pt-4">
			...
			{savedTokens?.some(
				...
				<>
					<TableItem
						key="asset-sscrt"
						token={savedTokens.find(
							(token) =>
								token.details.contract.address ===
								assets.sscrt.contract.address,
						)}
					/>
					<TableItem
						key="asset-test"
						token={savedTokens.find(
							(token) =>
								token.details.contract.address === assets.test.contract.address,
						)}
					/>
				</>
			) : (
			...
			{filteredTokens
				...
				.map((token) => (
					<TableItem
						key={`asset-${token.details.name}${(Math.random() + 1)
							.toString(36)
							.substring(3)}`}
						token={token}
					/>
