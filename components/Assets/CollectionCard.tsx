...
import type { Collection } from "../../stores/AssetStore";
import Image from "next/image";
import Link from "next/link";

export const CollectionCard: FunctionComponent<{ collection: Collection }> = ({
	collection,
}) => {
	return (
		<Link
			href={`/assets/${collection.details.contract.address}`}
			legacyBehavior
		>
			<div
				className="rounded-lg w-80 h-72 bg-gray-900 flex flex-col
            hover:shadow-lg hover:cursor-pointer transition duration-200"
			>
				<div className="h-3/5 w-full relative">
					<Image
						src={collection.details.banner}
					...
					<div className="h-20 w-20 shadow-2xl drop-shadow-2xl rounded-full absolute left-7 -bottom-8">
						<Image
							src={collection.details.logo}
							className="shadow-inner object-cover rounded-full"
				...
				<div className="w-full px-4 mb-5 h-1/5 flex flex-col justify-end">
					<h3 className="text-white">{collection.details.name}</h3>
