...
import { type TokenBalance, useBalanceStore } from "../../stores/BalanceStore";
	...
	return (
		<div className="w-full flex flex-col">
			<div className="grid grid-cols-5 items-center">
				...
				<div className="col-span-4 grid grid-flow-col auto-cols-max h-10 popup-asset-textbox items-center">
					<p className="text-white pl-2 pb-0.5">{assetData.name.unwrapped}</p>
					<input
						type="text"
						value={nativeAmount === 0 ? "" : nativeAmount}
						onChange={(e) => {
							handleChange(e, "native");
						}}
						maxLength={10}
						className="h-9 -mt-0.5 w-40 bg-transparent focus:ring-0 focus:border-transparent border-transparent"
					/>
					<div className="flex gap-x-2 items-center pr-2 pb-0.5">
						<p className="text-xs">Balance:</p>
						<p
							className="text-white text-sm hover:cursor-pointer"
							onKeyDown={(e) => e.key === "Enter" && handleChange(e, "native")}
							onClick={() => {
			...
			<div className="grid grid-cols-5 items-center">
				...
				<div className="col-span-4 grid grid-flow-col auto-cols-max h-10 popup-asset-textbox items-center">
					...
					<div className="flex gap-x-2 items-center pr-2 pb-0.5">
						...
						<p
							onKeyDown={(e) =>
								e.key === "Enter" && balances && setIbcAmount(balances.ibc)
							}
							className="text-white text-sm hover:cursor-pointer"
							onClick={() => {
			...
			<div className="grid grid-cols-5 items-center">
				...
				<div className="col-span-4 grid grid-flow-col auto-cols-max h-10 popup-asset-textbox items-center">
					...
					<input
						type="text"
						value={wrappedAmount === 0 ? "" : wrappedAmount}
						onChange={(e) => {
							handleChange(e, "wrapped");
						}}
						maxLength={10}
						className="h-9 -mt-0.5 w-40 bg-transparent focus:ring-0 focus:border-transparent border-transparent"
					/>
			...
			<div className="flex justify-between mx-auto w-2/3 pt-6 pb-2">
				<button
					type="button"
					className="py-2 px-4 w-24 bg-black border-[#D673FE] border rounded-lg hover:text-white duration-200 transition"
				>
					Unwrap
				</button>
				<button
					type="button"
					className="py-2 px-4 w-24 bg-black border-[#F2DC68] border rounded-lg hover:text-white duration-200 transition"
				>
					Transfer
				</button>
			</div>
	...
	return (
		<div className="w-full flex flex-col">
			...
			<div className="flex justify-between mx-auto w-2/3 pt-6 pb-2">
				<button
					type="button"
					className="py-2 px-4 w-24 bg-black border-[#68E6F2] border rounded-lg hover:text-white duration-200 transition"
				>
					Wrap
				</button>
				<button
...
export const WrapModal: FunctionComponent = () => {
	...
	const updateTokenBalance = useBalanceStore(
		(state) => state.updateTokenBalance,
	);
	...
	const getBalances = useCallback(
		async (fetchNew: boolean) => {
			let scrtBalance: TokenBalance | undefined;
			if (!fetchNew) {
				scrtBalance = tokenBalances.find(
					(tokenBalance) =>
						tokenBalance.address === assets.sscrt.contract.address,
				);
			}
			if (scrtBalance) {
				setBalances({
				...
				const scrtToken = savedTokens.find(
					(token) =>
						token.details.contract.address === assets.sscrt.contract.address,
				);
				if (scrtToken) {
					...
					if (
						tokenBalances.some(
							(tokenBalance) =>
								tokenBalance.address === newTokenBalance.address,
						)
					) {
	...
	return (
		<div className="w-full flex flex-col">
			<div className="grid grid-flow-col auto-cols-max h-10 popup-asset-textbox items-center">
				<p className="text-white pl-2 pb-0.5">SCRT</p>
				<input
					type="text"
					value={scrtAmount === 0 ? "" : scrtAmount}
					onChange={(e) => {
						handleChange(e, "scrt");
					}}
					maxLength={8}
				...
				<div className="flex gap-x-2 items-center pr-2 pb-0.5">
					...
					<p
						onKeyDown={(e) =>
							e.key === "Enter" && balances && setScrtAmount(balances.scrt)
						}
						className="text-white text-sm hover:cursor-pointer"
						onClick={() => {
