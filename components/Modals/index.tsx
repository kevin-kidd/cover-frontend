	...
	return (
		...
		>
			<Dialog
				...
				className="relative z-50 my-auto h-screen"
			>
				<div className="fixed inset-0 bg-black/30" aria-hidden="true" />
				<div className="fixed inset-0 flex items-center justify-center p-4">
					<div className="flex min-h-full items-center justify-center">
						<Dialog.Panel className="mx-auto max-w-96 bg-[#0D1822] rounded-xl shadow-xl">
							<div className="flex justify-between w-full pt-2 px-4">
								<p className="modal-title text-lg">{title}</p>
								<XIcon
									className="w-6 h-6 hover:cursor-pointer"
									onClick={() => setIsOpen(false)}
								/>
							</div>
							<div className="p-5 w-full">{modalContent}</div>
						</Dialog.Panel>
					</div>
				</div>
			</Dialog>
