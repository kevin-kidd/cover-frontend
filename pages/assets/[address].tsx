...
import { type Snip721Token, useAssetStore } from "../../stores/AssetStore";
...
const ViewCollectionPage: NextPage = () => {
	...
	const items = {
		left: [],
		right: [<SettingsWidget key="settings" page="test" />],
	};
	...
	const updateCollection = useAssetStore(
		(state) => state.assets.snip721.updateCollection,
	);
	const [inventory, setInventory] = useState<Snip721Token[]>([
		emptyNft,
		emptyNft,
		emptyNft,
	]);
	...
	if (collection?.details && collection.permit) {
		return (
			<>
				<Head>
					...
					<div className="lg:ml-sidebar w-full lg:w-auto h-screen flex flex-col">
						...
						<div className="px-1 sm:px-4 lg:container lg:mx-auto lg:px-20">
							<div className="flex flex-col border-t border-black mb-4 lg:mb-8 w-full px-2 sm:px-2 md:px-3 xl:px-6">
								...
								<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 items-center mt-10 gap-4">
									{inventory.map((nft, index) => (
										<Snip721Card
											nft={nft}
											contractAddress={collection.details.contract.address}
					...
					<div className="px-1 sm:px-4 lg:container lg:mx-auto lg:px-10 w-full h-full">
						<div className="border-t flex flex-col h-full justify-center items-center border-black">
							{collection && !collection.permit ? (
								<>
									<h1 className="-mt-20 text-2xl text-white">
										Permit not found.
									</h1>
									<h3 className="mt-2">
										Please
