...
export const assets = {
	...
	test: {
		...
		contract: {
			...
			codeHash:
				"338574ceb3062ffdefa28417e310d018bf045c9a5527a2e9901654a1e344e3c2",
		},
	},
	test_collection: {
		...
		banner:
			"https://stashhappstorage.blob.core.windows.net/collection-assets/secret173emcjes7992g4n76xafnzut9407f7a56ql3k0_banner_1655799060345.png",
		description:
			"The Council of Gyld is Legendao’s genesis NFT drop, featuring The Cryptids, mythical apes known as Sasquatch, Bigfoot and Yeti. The Cryptids knew that decentralized technology could bring their greatest wishes for humanity to fruition. They wanted to improve on Web 3 with a blockchain that preserved privacy for its users. Thus, the Secret Network was born.",
		contract: {
			address: "secret1usuwa2hvfz5me9gr42msjsffvj6xn20rm3ffrq",
			codeHash:
				"E38C18F5F54245E7C31A17ECC38F8D50B18D054B6DA975FE8C92579204682633",
		},
	},
};
//
...
const handleCollectionUpdate = (
	...
	const filteredCollections = collections.filter(
		(collection) =>
			...
			newCollection.details.contract.address,
	);
	...
	assets: {
		snip20: {
			...
			addToken: (token: Token) =>
				set((state: Draft<AssetsState>) => {
					state.assets.snip20.tokens = [...state.assets.snip20.tokens, token];
				}),
			...
			updateToken: (token: Token) =>
				set((state: Draft<AssetsState>) => {
					state.assets.snip20.tokens = handleTokenUpdate(
						token,
		...
		snip721: {
			...
			removeCollection: (address: string) =>
				set((state: Draft<AssetsState>) => {
					state.assets.snip721.collections =
						state.assets.snip721.collections.filter(
							(collection) => collection.details.contract.address !== address,
						);
				}),
			updateCollection: (collection: Collection) =>
				set((state: Draft<AssetsState>) => {
					state.assets.snip721.collections = handleCollectionUpdate(
						collection,
						state.assets.snip721.collections,
					);
				}),
